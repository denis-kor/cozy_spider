...
export function mountShop(root: HTMLElement, adapter: PlatformAdapter, hooks: ShopHooks = {}): Shop {
  ...
  function paint(): void {
    ...
    for (const sku of catalog.skus) {
      ...
      const applied = has && isApplied(sku.kind, sku.id)
      ...
      if (applied) {
        ...
        // Не куплен — ценник прямо на картинке, единственное место с ценой.
      ...
      if (has && !applied) {
        // Куплен (или бесплатный), но не «в игре» — предложить применить.
        // Смена сцены/колоды — перезагрузка (§ PAID-PACKS): выгружать и грузить
        // все текстуры Pixi вживую не стоит того.
        ...
        // Не куплен — покупка. Цена уже на картинке, на кнопке только действие.
        const action = document.createElement('button')
      ...
      // applied → без кнопки: активность показывают кант и медальон.
