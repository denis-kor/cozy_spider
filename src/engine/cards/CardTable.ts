...
export class CardTable {
  ...
  newGame(seed?: number, suits: SuitCount = this.game.suits): void {
    // Без явного сида берём «дружелюбный»: pickEasySeed выбирает лучший из
    // нескольких раскладов для текущего числа мастей (облегчены все три).
    // createDeal при этом не меняется, поэтому сейвы и отмена-реплеем
    // работают как раньше.
