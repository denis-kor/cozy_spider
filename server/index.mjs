 ...
 * доступ считается на лету от users.created_at. Истёк час — ручка прав
 * просто перестаёт отдавать эти sku, а клиент молча падает на бесплатный
 * пруд (resolvePacks). Куплены игроком за этот час — остаются навсегда.
 *
 * Выдаётся ровно один раз и никак не продлевается: точка отсчёта —
 * created_at аккаунта, в базе ничего не помечается, значит и сбрасывать
 * нечего. Перезайти или переоткрыть лавку, чтобы получить час заново,
 * невозможно by design.
 */
const TRIAL_SKUS = ['pack.tarot', 'deck.tarot']
const TRIAL_SECONDS = 3600
...
const routes = {
  ...
  'GET /api/entitlements': async (req, res) => {
    ...
    const owned = u
      ...
      : []
    // Приветственный триал: первый час новому аккаунту открыт «Салон таро».
    // Считаем от created_at — в базу ничего не пишем, поэтому по истечении
    // часа право гаснет само. То, что игрок успел купить, из fresh выпадает
